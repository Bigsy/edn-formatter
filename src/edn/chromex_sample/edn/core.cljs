...
(defn run-message-loop! [message-channel]
  ...
  (go-loop []
    (when-let [message (<! message-channel)]
      (do (log "wibbleedn") (process-message! message))
      (recur))
...
(defn click-handler [e]
  (let [p (dommy/value (sel1 :#why))]
  (.log js/console "You clicked my button! Congratulations" p)
  (dommy/set-text! (sel1 :#why1) p)
  ))
...
(defn init! []
  ...
  ;(create-editor (sel1 :#why1) {:linenumbers true})
  (dommy/listen! (sel1 :#openshort) :click click-handler))
